...
function generateSchedule(data) {
  ...
  function addEvent(startTime, endTime, userId, moduleId) {
    ...
    console.log(`Event added: ${startTime.toISOString()} - ${endTime.toISOString()} for user ${userId} with module ${moduleId}`);
  }
  ...
  function planModuleForDay(currentDay, remainingHours, modulesForDay) {
    ...
    while (currentTime.getHours() < 13 && morningRemainingHours > 0) {
      ...
      addEvent(currentTime, eventEnd, modulesForDay[0].user_id, modulesForDay[0].id);
    ...
    while (currentTime.getHours() < 17 && afternoonRemainingHours > 0) {
      ...
      addEvent(currentTime, eventEnd, modulesForDay[1].user_id, modulesForDay[1].id);
  ...
  while (currentDay <= new Date(availability[0].end)) {
    ...
    if (modulesForDay.length > 0) {
      remainingHours = planModuleForDay(currentDay, remainingHours, modulesForDay);
    }
