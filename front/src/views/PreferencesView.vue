      ...
      <form @submit.prevent="createPref" class="space-y-6">
        <div class="grid grid-cols-1 gap-6">
          <div>
            <label
              for="Intervenant"
              class="block text-sm font-medium text-gray-700 mb-2 font-poppins"
            >
              Intervenant
            </label>
            <select
      ...
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50 border-b">
            <tr>
              <th
                class="px-6 py-3 text-left text-sm font-medium text-gray-700 font-poppins"
              >
                Intervenant
              </th>
              <th
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 text-sm text-gray-700">
                <div
                  v-for="module in intervenant.modules"
                  :key="module.id"
                  class="font-poppins"
                >
                  {{ module.name }}
              ...
              <td class="px-6 py-4 text-sm text-gray-700 text-right">
                <div v-for="module in intervenant.modules" :key="module.id">
                  <span
                    v-for="(preference, index) in module.preferences"
                    :key="index"
                    class="font-poppins"
                  >
                    {{ preference
                    }}{{ index < module.preferences.length - 1 ? ", " : "" }}
                  </span>
                  <button
